...
export function ServiceItem({ service, barbershopName, barbershop }: ServiceItemProps) {
    ...
    const { data: availableTimeSlots, isLoading: isLoadingTimeSlots } = useQuery<string[]>({
        ...
        queryFn: () => fetchAvailableTimeSlots(token!, service.barbershopId, selectedDate!),
    ...
    const isConfirmDisabled = !selectedDate || !selectedTime || isBookingPending || isLoadingTimeSlots;
    ...
    const handleConfirm = async () => {
        if (!user || !token || !selectedTime || !selectedDate) {
        toast.error("Por favor, selecione uma data e um horário, e faça login.");
        return;
        }
                ...
                <div className="flex h-full flex-col gap-6">
                    ...
                    {selectedDate && (
                        ...
                        <div className="px-5 pb-6">
                            ...
                            >
                                {isBookingPending ? <Loader2Icon className="animate-spin" /> : "Confirmar"}
                            </Button>
                        </div>
                        </>
                    )}
